...
def clean_id_part(value: Any) -> str:
    """Convert value into a safe lowercase ID component."""
...
class ChromaEmbeddingPipelineTextOnly:
    ...
    def chunk_text(self, text: str, metadata: Dict[str, Any]) -> List[Tuple[str, Dict[str, Any]]]:
        ...
        separators = ["\n\n", "\n", ". ", "? ", "! ", " ", ""]
        splitter = RecursiveCharacterTextSplitter(chunk_size = self.chunk_size, chunk_overlap = self.chunk_overlap, separators = separators)
        chunks = splitter.create_documents(texts = [text], metadatas = [metadata])
        ...
        for i, doc in enumerate(chunks):
            ...
            results.append((doc.page_content, chunk_metadata))
        return results
    ...
    def get_embedding(self, text: str) -> List[float]:
        ...
        # TODO: Call OpenAI embeddings API
            ...
            response = self.openai_client.embeddings.create(model = self.embedding_model, input = input_text)
    ...
    def generate_document_id(self, file_path: Path, metadata: Dict[str, Any]) -> str:
        ...
        mission = metadata.get("mission") or file_path.parent.name
        source = metadata.get("source") or file_path.stem
        chunk_index = int(metadata.get("chunk_index", 0))
        mission_part = clean_id_part(mission)
        source_part = clean_id_part(source)
        return f"{mission_part}_{source_part}_chunk_{chunk_index:04d}"
    ...
    def add_documents_to_collection(self, documents: List[Tuple[str, Dict[str, Any]]], 
        ...
        self.silence_openai_api()
        
        if not documents:
            return {'added': 0, 'updated': 0, 'skipped': 0}
        
        if not update_mode in ['skip', 'update', 'replace']:
            raise ValueError("The parameter update_mode should be skip, update, or replace.")
        ...
        if update_mode == 'replace':
            try:
                self.collection.delete(where = {"source": file_path_str})
            except Exception:
                pass
            
        # TODO: Process documents in batches
        n_documents = len(documents)
        for batch_start_index in range(0, n_documents, batch_size):
            print(f'Documents processed {batch_start_index} of {n_documents}.')
            batch = documents[batch_start_index:batch_start_index + batch_size]
            ...
            for batch_index, document in enumerate(batch):
                ...
                if type(metadata) != dict:
                    metadata = {}
                metadata.setdefault("source", file_path_str)
                metadata.setdefault("file_name", file_path.name)
                metadata.setdefault("chunk_index", batch_start_index + batch_index)
                ...
                if exists and update_mode == "update":
                    updating_ids.append(doc_id)
                    updating_texts.append(text)
                    updating_embeddings.append(embedding)
                    updating_metadatas.append(metadata)
                else:
                    adding_ids.append(doc_id)
                    adding_texts.append(text)
                    adding_embeddings.append(embedding)
                    adding_metadatas.append(metadata)
                
            #   - Add or update in collection
            if adding_ids:
                self.collection.add(ids = adding_ids, documents = adding_texts, embeddings = adding_embeddings, metadatas = adding_metadatas)
                stats["added"] += len(adding_ids)
            if updating_ids:
                self.collection.update(ids = updating_ids, documents = updating_texts, embeddings = updating_embeddings, metadatas = updating_metadatas)
                stats["updated"] += len(updating_ids)
    ...
    def process_all_text_data(self, base_path: str, update_mode: str = 'skip') -> Dict[str, int]:
        ...
        text_extensions = {".txt", ".md", ".html", ".csv", ".xml", ".json"}
        files_to_process = [path for path in base_path.rglob("*") if path.is_file() and path.suffix.lower() in text_extensions]
        ...
        for file_path in files_to_process:
            try: 
                ...
                # Infer mission name from parent folder
                mission_name = file_path.parent.name
                
                if mission_name not in stats["missions"]:
                    stats["missions"][mission_name] = {
                        "files_processed": 0,
                        "documents_added": 0,
                        "documents_updated": 0,
                        "documents_skipped": 0,
...
#                     stats["missions"][mission_name]["documents_skipped"] += 1
#                     continue
...
#                     continue
                ...
                add_stats = self.add_documents_to_collection(documents = documents, file_path = file_path, update_mode = update_mode)
                added = add_stats.get("added", 0)
                updated = add_stats.get("updated", 0)
                skipped = add_stats.get("skipped", 0)
                ...
                stats["missions"][mission_name]["files_processed"] += 1
                stats["missions"][mission_name]["documents_added"] += added
                stats["missions"][mission_name]["documents_updated"] += updated
                stats["missions"][mission_name]["documents_skipped"] += skipped
                stats["missions"][mission_name]["total_chunks"] += len(documents)
            ...
            # TODO: Handle errors gracefully
                ...
                stats["missions"][mission_name]["errors"] += 1
...
def main():
    ...
    for mission, mission_stats in stats['missions'].items():
        #print(f'mission_stats.keys(): {mission_stats.keys()}')
        #logger.info(f"  {mission}: {mission_stats['files']} files, {mission_stats['chunks']} chunks")
        logger.info(f"  {mission}: {mission_stats['files_processed']} files, {mission_stats['total_chunks']} chunks")
        #logger.info(f"    Added: {mission_stats['added']}, Updated: {mission_stats['updated']}, Skipped: {mission_stats['skipped']}")
        logger.info(f"    Added: {mission_stats['documents_added']}, Updated: {mission_stats['documents_updated']}, Skipped: {mission_stats['documents_skipped']}")
